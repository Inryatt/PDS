...
public class JGalo extends JFrame implements ActionListener {
	...
	public static void main(String args[]) {
		//If the user passes 'O' as an argument, they'll start the game with 'O'
		//Anything else, starts with 'X' (Also 'X' if no argument is provided.)
		if(args.length > 0 && args[0].equals("O")) {
			new JGalo(new Galinha(false)); // Começar com 'O'
		}
		else {
			new JGalo(new Galinha(true)); // Começar com X (default)
		}
	}
} 
