...
public class WSGenerator {
    public static void main(String[] args) {
                    ...
                    try {
                        try {
                            ...
                            if (args[i + 1].contains(".txt")) //Adds the file extension if only the
                                ...
                                input = args[i + 1] + ".txt";
                            file = new File(".\\lab01\\WSGin\\" + input);
                        ...
                        //If the user includes the option but not the argument, this would go out of bounds
        ...
        //Debug: System.out.print("Input file is: " + file.getName() + "; Output file is: " + output + "; Size is: " + size + "\n");
        ...
        while (read.hasNextLine()) {
            ...
            for (String w : tmp_words) {
                if (w.length() > size) { //If the word is larger than size it won't fit the soup in any way
                    System.err.print("ERROR: Word in input file is bigger than the specified size allows. Try increasing the size argument.\n");
                    return;
                }
                if (!w.isEmpty())
                    words.add(w);
        ...
        if (soup[0][0] == '+') {
            ...
            File out = new File(".\\lab01\\WSGout\\" + output);
            try {
    ...
    public static char[][] wordInsertion ( char[][] soup, ArrayList<String > words,int curIdx, int len){
        ...
        if (curIdx == words.size()) { //If all words have been inserted
            ...
            for (int i : iVals) {
                for (int j : jVals) {
                    if (!(soup[i][j] == word.charAt(0) || soup[i][j] == '*')) continue; //space is occupied
                    else {
                        ...
                        for (Direction dir : dirVals) {
                            ...
                            if (dir.getX() > 0 && j + wordLen > len) continue;
                                ...
                                for (int k = 1; k < wordLen; k++) {
                                    if(soup[searchY][searchX] == '*') overlaps = false;
                                ...
                                if (fits && !overlaps) {
                                    ...
                                    if (tmpSoup[0][0] == '+') fits = false; //If an error occurred, the first character
                                    //will be a + -> Couldn't fit in all words
                                    else soup = tmpSoup;
                                }
                                else fits = false;
                                if (fits) break; //Only reached if all words fit in -> Solution found!
                            }
                        }
                    }
                    if (fits) break;
